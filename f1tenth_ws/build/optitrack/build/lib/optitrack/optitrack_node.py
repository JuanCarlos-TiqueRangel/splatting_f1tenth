...
class OptiTrackMultiOdometryNode(Node):
    def __init__(self):
        super().__init__("optitrack_multi_odometry_node")

        # Parameters
        self.declare_parameter("client_ip", "192.168.0.37")
        self.declare_parameter("server_ip", "192.168.0.4")
        self.declare_parameter("robot_ids", [530, 531])
        ...
        # Publishers per object
        self.odom_pubs = {}
        self.rpy_pubs = {}
        for rid in self.robot_ids:
            self.odom_pubs[rid] = self.create_publisher(Odometry, f"/optitrack/object_{rid}/odom", 10)
            self.rpy_pubs[rid] = self.create_publisher(Vector3, f"/optitrack/object_{rid}/rpy", 10)
        ...
        self.get_logger().info(f"Publishing objects: {self.robot_ids}")
    ...
    def _publish_one_object(self, rid: int, s: Sample):
        ...
        if self.alpha > 0.0:
            self._v_filt[rid] = tuple(self.alpha * self._v_filt[rid][i] + (1.0 - self.alpha) * v[i] for i in range(3))
            self._w_filt[rid] = tuple(self.alpha * self._w_filt[rid][i] + (1.0 - self.alpha) * w[i] for i in range(3))
    ...
    def _timer_cb(self):
        ...
        for rid, s in latest_copy.items():
            ...
            self._publish_one_object(rid, s)
...
def main():
    rclpy.init()
    node = OptiTrackMultiOdometryNode()
    try:
        rclpy.spin(node)
